...
public class Simulator
{
    ...
    private int ReduceCrumbsCounter=0;
    /**
    ...
    {
        for(int tick = 1;   tick <= numSteps && ( vehiclesCarryingRock(vehicles) || view.anyRocksAvialble() )&& view.isViable(field) ; tick++) {
    ...
    {
    	if(ReduceCrumbsCounter<80) {
    		ReduceCrumbsCounter++;
    	}else {
    		field.reduceCrumbs();
    		ReduceCrumbsCounter=0;
    	}
        step++;
   ...
   /**
    ...
    public boolean vehiclesCarryingRock(ArrayList<Vehicle> vehicles) {
    	
    	for(Vehicle v:vehicles) {
    		
    		if(v.carryingSample==true) {
    			return true;
    		}
    	}
    	return false;
